...
class MoveTestCase(unittest.TestCase, ABC):
    def assert_move(
        self,
        board: chess.Board,
        expected_move: Union[chess.Move, str],
        ...
        expected_msg = (
            f"EXPECTED {fmt_legal(expected_legal)} move {expected_move.uci()}"
        )
        actual_msg = (
            f"ACTUAL {fmt_legal(legal)} move {actual_move and actual_move.uci()}"
        )
    ...
    def assert_moves(
        self,
        board: chess.Board,
        moves: Iterable[Union[chess.Move, str]],
        legal: bool = True,
    ):
        for move in moves:
...
def make_test_move(board: chess.Board, move: chess.Move):
    if move in board.legal_moves:
        ...
        if piece:
            ...
            board.set_piece_at(
                move.to_square,
                chess.Piece(move.promotion, piece.color) if move.promotion else piece,
            )
...
class TestOpeningWhite(MoveTestCase):
    ...
    def test_invalid_pawn_capture(self):
        moves = [
            "a2b3",
            "a2c3",  # Invalid pawn captures by a2
            "b2a3",
            "b2c3",  # Invalid pawn captures by b2
            "c2b3",
            "c2d3",  # Invalid pawn captures by c2
            "d2c3",
            "d2e3",  # Invalid pawn captures by d2
            "e2d3",
            "e2f3",  # Invalid pawn captures by e2
            "f2e3",
            "f2g3",  # Invalid pawn captures by f2
            "g2f3",
            "g2h3",  # Invalid pawn captures by g2
            "h2g3",
            "h2f3",  # Invalid pawn captures by h2
        ]
        self.assert_moves(self.board, moves, legal=False)
...
class TestOpeningBlack(MoveTestCase):
    ...
    def test_invalid_pawn_capture(self):
        moves = [
            "a7b6",
            "a7c6",  # Invalid pawn captures by a7
            "b7a6",
            "b7c6",  # Invalid pawn captures by b7
            "c7b6",
            "c7d6",  # Invalid pawn captures by c7
            "d7c6",
            "d7e6",  # Invalid pawn captures by d7
            "e7d6",
            "e7f6",  # Invalid pawn captures by e7
            "f7e6",
            "f7g6",  # Invalid pawn captures by f7
            "g7f6",
            "g7h6",  # Invalid pawn captures by g7
            "h7g6",
            "h7f6",  # Invalid pawn captures by h7
        ]
        self.assert_moves(self.board, moves, legal=False)
...
class TestEnPassantWhite(MoveTestCase):
    def setUp(self):
        self.board = chess.Board(
            "rnbqkbnr/pppp1ppp/8/4pP2/8/8/PPPP2PP/RNBQKBNR w KQkq e6 0 1"
        )
...
class TestEnPassantBlack(MoveTestCase):
    def setUp(self):
        self.board = chess.Board(
            "rnbqkbnr/pppp2pp/8/8/4Pp2/8/PPPP1PpP/RNBQKBNR b KQkq e3 0 1"
        )
...
class TestPromotionWhite(MoveTestCase):
    ...
    def test_invalid_promotion_upwards(self):
        self.assert_move(
            self.board, "g7g8k", expected_legal=False
        )  # Promoting to king is invalid
        self.assert_move(
            self.board, "g7g8p", expected_legal=False
        )  # Promoting to pawn is invalid
    ...
    def test_invalid_promotion_capture(self):
        self.assert_move(
            self.board, "e7f8q", expected_legal=False
        )  # Capturing on f8 is invalid
        self.assert_move(
            self.board, "e7d8k", expected_legal=False
        )  # Invalid promotion piece (king)

    def test_pawn_moves_upwards_with_king(self):
        self.assert_move(
            self.board, "e7e8k", expected_legal=False
        )  # Invalid promotion piece (king)
        self.assert_move(
            self.board, "e7e8p", expected_legal=False
        )  # Invalid promotion piece (pawn)
...
class TestPromotionBlack(MoveTestCase):
    ...
    def test_invalid_promotion_upwards(self):
        self.assert_move(
            self.board, "g2g1k", expected_legal=False
        )  # Promoting to king is invalid
        self.assert_move(
            self.board, "g2g1p", expected_legal=False
        )  # Promoting to pawn is invalid
    ...
    def test_invalid_promotion_capture(self):
        self.assert_move(
            self.board, "e2f1q", expected_legal=False
        )  # Capturing on f1 is invalid
        self.assert_move(
            self.board, "e2d1k", expected_legal=False
        )  # Invalid promotion piece (king)

    def test_pawn_moves_upwards_with_king(self):
        self.assert_move(
            self.board, "e2e1k", expected_legal=False
        )  # Invalid promotion piece (king)
        self.assert_move(
            self.board, "e2e1p", expected_legal=False
        )  # Invalid promotion piece (pawn)
