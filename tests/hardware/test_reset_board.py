...
class TestResetBoard(RobotTestCase):
    ...
    def test_unnecessary_pawn(self):
        ...
        current_board.set_piece_at(
            chess.parse_square("e3"), chess.Piece(chess.PAWN, chess.WHITE)
        )
    ...
    def test_cyclic_dependencies(self):
        ...
        starting_board.set_piece_at(
            chess.parse_square("b1"), chess.Piece(chess.KNIGHT, chess.WHITE)
        )
        starting_board.set_piece_at(
            chess.parse_square("c3"), chess.Piece(chess.BISHOP, chess.WHITE)
        )
        starting_board.set_piece_at(
            chess.parse_square("e4"), chess.Piece(chess.PAWN, chess.WHITE)
        )

        ending_board = chess.Board()
        ending_board.clear()
        ending_board.set_piece_at(
            chess.parse_square("c3"), chess.Piece(chess.KNIGHT, chess.WHITE)
        )
        ending_board.set_piece_at(
            chess.parse_square("e4"), chess.Piece(chess.BISHOP, chess.WHITE)
        )
        ending_board.set_piece_at(
            chess.parse_square("b1"), chess.Piece(chess.PAWN, chess.WHITE)
        )
